...
class CausalSelfAttention(nn.Module, AttachableStore):
    def __init__(
        ...
        # TODO @thomasw21: refactor so that we store that default in a single place.
        tp_mode = parallel_config.tp_mode if parallel_config is not None else TensorParallelLinearMode.ALL_REDUCE
        # print(f"===Guanhua CausalSelfAttn {tp_mode=}")
        tp_linear_async_communication = (
            parallel_config.tp_linear_async_communication if parallel_config is not None else False
        )
...
class LlamaDecoderLayer(nn.Module):
    ...
    def _core_forward(
        ...
        hidden_states0 = self.input_layernorm(hidden_states0) # [seq, batch, hidden]
        hidden_states1 = self.input_layernorm(hidden_states1)
        # print(f"===Guanhua {hidden_states.shape=}")
        # guanhua
        # hidden_states0, hidden_states1 = hidden_states.chunk(2, dim=1)
        ...
        hidden_states = torch.cat((hidden_states0,hidden_states1), dim=1)
        output = torch.cat((output0["sequence_mask"],output1["sequence_mask"]), dim=0)
        return hidden_states, output #output["sequence_mask"]
...
class LlamaModel(nn.Module):
    ...
    def __init__(
        ...
        # print(f"===Guanhua LlamaModel {self.tp_mode=}")
        tp_linear_async_communication = (
            parallel_config.tp_linear_async_communication if parallel_config is not None else False
        )
